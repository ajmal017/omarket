function result = johansen_modified(x,p,k)
...
sig   = sk0*inv(s00)*(sk0');
tmp   = inv(skk);
if(sum(sum(isnan(tmp*sig)))>0 || sum(sum(isinf(tmp*sig)))>0)
    result = [];
    return
else
    [du au] = eig(tmp*sig);
